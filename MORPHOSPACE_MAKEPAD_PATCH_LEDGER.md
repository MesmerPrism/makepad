...
| XR Vulkan f32 mesh-SDF compute proof plumbing | `platform/src/cx_api.rs`, `platform/src/os/linux/vulkan.rs`, `platform/src/os/linux/vulkan/mesh_sdf_probe.rs` | Generic XR adapter proof that skins a supplied f32 vertex buffer, consumes a supplied triangle buffer, writes a dense SDF buffer, and reports bounded readback metrics plus renderer-lifetime program and source mesh buffer reuse. | Keep local unless a generic Makepad compute-proof API becomes upstreamable. This is not Matter/SDF authority; downstream crates own CPU oracle semantics and marker contracts. | `python tools\makepad_fork_format.py --changed --check`; `cargo check -p makepad-platform`; downstream Quest-Makepad and Hostess checks for marker propagation. |
