...
| XR Vulkan f32 stimulus-volume compute/readback previews | `platform/src/cx_api.rs`, `platform/src/os/linux/android/android.rs`, `platform/src/os/linux/vulkan.rs`, `platform/src/os/linux/vulkan/volume_probe.rs`, `platform/src/os/linux/vulkan/volume_raymarch_preview.rs` | Generic XR adapter proofs that run bounded f32 volume point-sample and tiny stereo raymarch-preview compute shaders, then return compact storage-buffer readback metrics to downstream app-shell evidence. | Keep local unless a generic Makepad compute/readback preview API becomes upstreamable. This is not Optics profile authority, not a production volume renderer, and not a runtime texture-binding claim. | `python tools\makepad_fork_format.py --changed --check`; `cargo check -p makepad-platform`; downstream Quest-Makepad and Hostess checks for profile/oracle marker propagation with `gpuComputeReady=false`. |
| XR Vulkan f32 mesh-SDF compute proof plumbing | `platform/src/cx_api.rs`, `platform/src/os/linux/android/android.rs`, `platform/src/os/linux/vulkan.rs`, `platform/src/os/linux/vulkan/mesh_sdf_probe.rs`, `platform/src/os/linux/vulkan/field_sample_probe.rs`, `platform/src/os/linux/vulkan/field_force_sample_probe.rs` | Generic XR adapter proof that skins a supplied f32 vertex buffer, consumes a supplied triangle buffer, writes a dense SDF buffer, reports a compact eight-sample readback window, exposes bounded resident-field distance sampling, and exposes bounded resident-field force sampling. Renderer-lifetime program reuse, source mesh buffer residency, and capacity-aware derived skinned-position/dense-SDF buffer reuse remain Makepad resource evidence only. Field sampling and field-force sampling validate the logical grid shape against a resident capacity buffer; capacity bytes are resource evidence, not voxel-count authority. | Keep local unless a generic Makepad compute-proof API becomes upstreamable. This is not Matter/SDF/particle authority; downstream crates own CPU oracle semantics, field/force marker contracts, and force-mode selection. | `python tools\makepad_fork_format.py --changed --check`; `cargo check -p makepad-platform`; downstream Quest-Makepad and Hostess checks for marker propagation. |
...
4. Platform/video watchlist:
   ...
   - `platform/src/os/linux/vulkan.rs`: leave alone unless HWB/video-import
     ...
     validation gate. Video import/lifetime work should start in
     `platform/src/os/linux/vulkan/video_hardware_buffer.rs` or
     `platform/src/os/linux/vulkan/texture_lifetime.rs` before expanding the
