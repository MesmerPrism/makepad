...
| Desktop renderer cache, idle, and live-reload intake | `platform/script/src/shader_control.rs`, `platform/src/os/windows/d3d11.rs`, `libs/windows/windows-rs/src/Windows/mod.rs`, `platform/src/os/linux/select_timer.rs`, `platform/src/os/linux/opengl_cx.rs`, `platform/src/os/linux/x11/linux_x11.rs`, `platform/src/os/linux/wayland/linux_wayland.rs`, `platform/src/live_reload.rs` | Small upstream rendering/app-shell fixes: `#979` GLSL/X11 window-position fixes, `#987/#988` Windows shader bytecode cache/compile-set optimizations, `#1103` Linux idle/vsync hardening, and `#1124` hot-reload static-site filtering. | Prefer upstream for generic renderer and app-shell fixes. Keep local async HLSL compile hardening and Morphospace guard signatures as fork maintenance, not app authority. | `python tools\makepad_fork_format.py --changed --check`; `python tools\check_morphospace_makepad_guards.py`; `cargo check -p makepad-platform`; Linux desktop compile/runtime idle validation when a Linux release path matters. |
...
- 2026-06-16: completed a Makepad integration follow-up against current
  upstream PR state. Verified `#979`, `#987`, and `#988` are already present
  in this fork; imported the remaining narrow `#1103` Linux desktop idle/vsync
  pieces and the merged `#1124` hot-reload `script_apply_eval!` static-site
  filter; added guard signatures for `#979/#987/#988/#1103/#1124`. Deferred
  merged `#1085/#1109` to a focused cross-platform IME slice, kept open
  `#1060` as future state-preserving reload work, kept `#929` as media
  decomposition reference only, and rejected broad `#1112` beyond the already
  adapted Android OpenGL OES companion fallback.
