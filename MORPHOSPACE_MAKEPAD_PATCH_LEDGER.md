...
| Vulkan texture resources, OpenXR targets, frame resources, pipeline resources, hardware-buffer import, and lifetime | `platform/src/os/linux/vulkan.rs`, `platform/src/os/linux/vulkan/frame_resources.rs`, `platform/src/os/linux/vulkan/openxr_targets.rs`, `platform/src/os/linux/vulkan/pipeline_resources.rs`, `platform/src/os/linux/vulkan/texture_resources.rs`, `platform/src/os/linux/vulkan/texture_lifetime.rs`, `platform/src/os/linux/vulkan/video_hardware_buffer.rs`, `platform/src/os/linux/vulkan_naga.rs`, shader reflection/lowering files | Renderer correctness and external video import evidence. Reusable frame buffer/descriptors live in `frame_resources.rs`; OpenXR multiview target/session/readback/draw ownership lives in `openxr_targets.rs`; graphics pipeline keys, immutable video sampler keying, shader descriptor layouts, shader modules, graphics pipeline creation, and Makepad sampler creation live in `pipeline_resources.rs`; vector texture upload, render/depth target resources, depth dummy resources, and image layout transitions live in `texture_resources.rs`. Texture-resource retirement, video hardware-buffer cache helpers, AHardwareBuffer import, external YCbCr sampler setup, and video texture update entry points live in Vulkan child modules; the root Vulkan file remains lifecycle/window-swapchain/render-pass/draw-list/geometry orchestration. | Upstream only as small generic texture/resource/lifetime/sampler/reflection fixes. Do not broad-split upstream-owned files without a rebase plan. | `python tools\makepad_fork_format.py --changed --check`; `python tools\check_morphospace_makepad_guards.py`; `cargo check -p makepad-platform --target aarch64-linux-android` when touching Android Vulkan; downstream GPU/page-fault gate for behavior changes. |
...
4. Platform/video watchlist:
  ...
  - `platform/src/os/linux/vulkan.rs`: root basic storage/u32/f32 probe code is
    ...
    per-frame buffer/descriptors are split to
    `platform/src/os/linux/vulkan/frame_resources.rs`, OpenXR
    multiview target/session/readback/draw ownership is split to
    `platform/src/os/linux/vulkan/openxr_targets.rs`, graphics pipeline and
    ...
    focused on instance/device lifecycle, window swapchain, render-pass,
    draw-list, and geometry orchestration unless HWB/video-import
    work or a narrow proof-program/resource-lifetime slice forces a clear
