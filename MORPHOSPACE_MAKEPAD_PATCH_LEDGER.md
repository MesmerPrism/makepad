...
4. Platform/video watchlist:
  ...
  - `platform/src/os/linux/vulkan.rs`: root basic storage/u32/f32 probe code is
    split to `platform/src/os/linux/vulkan/basic_compute_probe.rs`, and
    texture/image resource ownership is split to
    `platform/src/os/linux/vulkan/texture_resources.rs`. Leave the root file
    focused on lifecycle, swapchain, render-pass, draw-list, pipeline, and
    geometry orchestration unless HWB/video-import work or a narrow
    proof-program/resource-lifetime slice forces a clear resource-lifetime,
    command-buffer, descriptor, or pipeline boundary with a GPU validation
    gate. Video import/lifetime work should start in
