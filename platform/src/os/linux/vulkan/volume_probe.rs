...
const XR_GPU_F32_VOLUME_PROBE_ENTRY: &str = "compute_main";
const XR_GPU_F32_VOLUME_PROBE_WGSL: &str = r#"
struct VolumeProbeSample {
    uv_eye_time: vec4<f32>,
    ray_origin_depth: vec4<f32>,
...
struct VolumeProbeOutput {
    rgba: vec4<f32>,
    density_depth_status: vec4<f32>,
};

@group(0) @binding(0) var<storage, read> input_samples: array<VolumeProbeSample, 8>;
@group(0) @binding(1) var<storage, read_write> outputs: array<VolumeProbeOutput, 8>;

fn triangle_wave(value: f32) -> f32 {
    return abs(fract(value) * 2.0 - 1.0);
}

fn volume_probe_output(sample: VolumeProbeSample) -> VolumeProbeOutput {
    let uv = sample.uv_eye_time;
    let origin = sample.ray_origin_depth.xyz;
    let depth = sample.ray_origin_depth.w;
    let direction = sample.ray_direction_step.xyz;
    let p = origin + direction * depth;
    let frequency = max(sample.volume_params.x, 0.001);
    let phase = sample.volume_params.y;
    let opacity = clamp(sample.volume_params.z, 0.0, 4.0);
    let wave_a =
...
fn compute_main(@builtin(global_invocation_id) id: vec3<u32>) {
    let index = id.x;
    if (index < 8u) {
        outputs[index] = volume_probe_output(input_samples[index]);
    }
}
"#;

fn expected_xr_gpu_f32_volume_probe_outputs(
    samples: [XrGpuF32VolumeProbeSample; XR_GPU_F32_VOLUME_PROBE_SAMPLES],
) -> [XrGpuF32VolumeProbeOutput; XR_GPU_F32_VOLUME_PROBE_SAMPLES] {
    let mut expected = [XrGpuF32VolumeProbeOutput::default(); XR_GPU_F32_VOLUME_PROBE_SAMPLES];
    for (index, sample) in samples.iter().copied().enumerate() {
        expected[index] = XrGpuF32VolumeProbeOutput {
            rgba: sample.expected_rgba,
            density_depth_status: sample.expected_density_depth_status,
        };
    }
    expected
}

pub(super) struct VulkanXrF32VolumeProbeResources {
    request_id: u64,
    started: Instant,
    samples: [XrGpuF32VolumeProbeSample; XR_GPU_F32_VOLUME_PROBE_SAMPLES],
    expected_outputs: [XrGpuF32VolumeProbeOutput; XR_GPU_F32_VOLUME_PROBE_SAMPLES],
    sample_count: usize,
    tolerance: f32,
...
impl CxVulkan {
    pub(crate) fn submit_xr_f32_volume_probe(
        &mut self,
        samples: [XrGpuF32VolumeProbeSample; XR_GPU_F32_VOLUME_PROBE_SAMPLES],
        sample_count: usize,
        tolerance: f32,
    ) -> Result<XrGpuF32VolumeProbeResult, String> {
        let ticket = self.submit_xr_f32_volume_probe_async(samples, sample_count, tolerance)?;
        self.wait_xr_f32_volume_probe(ticket.request_id)
    }

    pub(crate) fn submit_xr_f32_volume_probe_async(
        &mut self,
        samples: [XrGpuF32VolumeProbeSample; XR_GPU_F32_VOLUME_PROBE_SAMPLES],
        sample_count: usize,
        tolerance: f32,
    ) -> Result<XrGpuF32VolumeProbeTicket, String> {
        let started = Instant::now();
        let sample_count = sample_count.min(XR_GPU_F32_VOLUME_PROBE_SAMPLES);
        let tolerance = if tolerance.is_finite() && tolerance >= 0.0 {
            tolerance
        } else {
            0.0
        };
        let expected_outputs = expected_xr_gpu_f32_volume_probe_outputs(samples);
        let input_byte_len = std::mem::size_of::<
            [XrGpuF32VolumeProbeSample; XR_GPU_F32_VOLUME_PROBE_SAMPLES],
        >() as vk::DeviceSize;
        ...
        let shader_spv = compile_compute_wgsl_to_spirv(
            XR_GPU_F32_VOLUME_PROBE_WGSL,
            XR_GPU_F32_VOLUME_PROBE_ENTRY,
        )?;
        let input =
        ...
        let shader_module =
            match unsafe { self.device.create_shader_module(&shader_module_info, None) } {
                ...
                Err(err) => {
                    ...
                    self.destroy_buffer(input);
                    return Err(format!(
                        "create_shader_module(f32 volume probe) failed: {err:?}"
                    ));
                }
            };
        ...
        let descriptor_set_layout = match unsafe {
            ...
            Err(err) => {
                ...
                self.destroy_buffer(input);
                return Err(format!(
                    "create_descriptor_set_layout(f32 volume probe) failed: {err:?}"
                ));
            }
        };
        ...
        let pipeline_layout = match unsafe {
            ...
            Err(err) => {
                ...
                self.destroy_buffer(input);
                return Err(format!(
                    "create_pipeline_layout(f32 volume probe) failed: {err:?}"
                ));
            }
        };

        let entry = std::ffi::CString::new(XR_GPU_F32_VOLUME_PROBE_ENTRY).unwrap();
        ...
        let compute_pipeline = match unsafe {
            ...
            Err((pipelines, err)) => {
                ...
                self.destroy_buffer(input);
                return Err(format!(
                    "create_compute_pipelines(f32 volume probe) failed: {err:?}"
                ));
            }
        };
        ...
        let descriptor_pool = match unsafe {
            ...
            Err(err) => {
                ...
                self.destroy_buffer(input);
                return Err(format!(
                    "create_descriptor_pool(f32 volume probe) failed: {err:?}"
                ));
            }
        };
        let descriptor_set = {
            ...
            match unsafe { self.device.allocate_descriptor_sets(&alloc_info) } {
                ...
                Err(err) => {
                    ...
                    self.destroy_buffer(input);
                    return Err(format!(
                        "allocate_descriptor_sets(f32 volume probe) failed: {err:?}"
                    ));
                }
            }
        };
        ...
        let command_buffer = {
            ...
            match unsafe { self.device.allocate_command_buffers(&alloc_info) } {
                ...
                Err(err) => {
                    ...
                    self.destroy_buffer(input);
                    return Err(format!(
                        "allocate_command_buffers(f32 volume probe) failed: {err:?}"
                    ));
                }
            }
        };
        let fence = {
            ...
            match unsafe { self.device.create_fence(&fence_info, None) } {
                ...
                Err(err) => {
                    ...
                    self.destroy_buffer(output);
                    self.destroy_buffer(input);
                    return Err(format!("create_fence(f32 volume probe) failed: {err:?}"));
                }
            }
        };
        ...
        let command_result = (|| -> Result<(), String> {
            unsafe {
                self.device
                    ...
                    .map_err(|e| format!("begin_command_buffer(f32 volume probe) failed: {e:?}"))?;
                ...
                self.device
                    ...
                    .map_err(|e| format!("end_command_buffer(f32 volume probe) failed: {e:?}"))?;
                self.device
                    .queue_submit(
                        self.queue,
                    ...
                    .map_err(|e| format!("queue_submit(f32 volume probe) failed: {e:?}"))?;
        ...
        let resource_generation = self.xr_f32_volume_probe_resources.len() as u64 + 1;
        let request_id = queue_submit_serial;
        self.xr_f32_volume_probe_resources
            .push(VulkanXrF32VolumeProbeResources {
                request_id,
                started,
                samples,
                expected_outputs,
        ...
        let retained_resource_count = self.xr_f32_volume_probe_resources.len();
        let pending_retire_count = retained_resource_count;

        Ok(XrGpuF32VolumeProbeTicket {
            request_id,
            queue_submit_serial,
            resource_generation,
    ...
    pub(crate) fn poll_xr_f32_volume_probe(
        &mut self,
        request_id: u64,
    ) -> Result<Option<XrGpuF32VolumeProbeResult>, String> {
        let Some(resource_index) = self
            .xr_f32_volume_probe_resources
            .iter()
            .position(|resource| resource.request_id == request_id)
        else {
            return Ok(None);
        };
        if self.xr_f32_volume_probe_resources[resource_index].completed {
            return Ok(None);
        }

        let fence = self.xr_f32_volume_probe_resources[resource_index].fence;
        let queue_submit_serial =
            self.xr_f32_volume_probe_resources[resource_index].queue_submit_serial;
        match unsafe { self.device.get_fence_status(fence) } {
            Ok(true) => {
                ...
                self.complete_xr_f32_volume_probe(resource_index, false)
                    .map(Some)
            }
            Ok(false) => Ok(None),
            Err(err) => Err(format!(
                "get_fence_status(f32 volume probe {request_id}) failed: {err:?}"
            )),
        }
    }

    fn wait_xr_f32_volume_probe(
        &mut self,
        request_id: u64,
    ) -> Result<XrGpuF32VolumeProbeResult, String> {
        let resource_index = self
            ...
            .iter()
            .position(|resource| resource.request_id == request_id)
            .ok_or_else(|| format!("f32 volume probe {request_id} was not found"))?;
        if self.xr_f32_volume_probe_resources[resource_index].completed {
            return Err(format!(
                "f32 volume probe {request_id} was already completed"
            ));
        }
        ...
        unsafe {
            self.device
                ...
                .map_err(|e| format!("wait_for_fences(f32 volume probe) failed: {e:?}"))?;
            self.device
                .queue_wait_idle(self.queue)
                .map_err(|e| format!("queue_wait_idle(f32 volume probe) failed: {e:?}"))?;
        }
        self.gpu_completed_submit_serial =
            self.gpu_completed_submit_serial.max(queue_submit_serial);
        self.collect_retired_texture_resources();
        self.complete_xr_f32_volume_probe(resource_index, true)
    }

    fn complete_xr_f32_volume_probe(
        &mut self,
        ...
        let (
            ...
            let resource = self
                .xr_f32_volume_probe_resources
                .get(resource_index)
                .ok_or_else(|| "f32 volume probe resource index is stale".to_string())?;
            if resource.completed {
                return Err(format!(
                    "f32 volume probe {} was already completed",
                    resource.request_id
                ));
            }
            (
        ...
        let outputs = unsafe {
            let mapped = self
                ...
                .map_err(|err| format!("map_memory(f32 volume probe readback) failed: {err:?}"))?;
            let rows = std::slice::from_raw_parts(
                mapped as *const XrGpuF32VolumeProbeOutput,
                XR_GPU_F32_VOLUME_PROBE_SAMPLES,
            );
            let mut outputs =
                [XrGpuF32VolumeProbeOutput::default(); XR_GPU_F32_VOLUME_PROBE_SAMPLES];
        ...
        if let Some(resource) = self.xr_f32_volume_probe_resources.get_mut(resource_index) {
        ...
        Ok(XrGpuF32VolumeProbeResult {
            samples,
            outputs,
            expected_outputs,
            sample_count,
            component_count: sample_count * 8,
    ...
    pub(super) fn destroy_xr_f32_volume_probe_resources(&mut self) {
        let resources = std::mem::take(&mut self.xr_f32_volume_probe_resources);
