...
use crate::{
    ...
    cx_api::{
        ...
        XrGpuF32SkinningProbeSample, XrGpuStorageBufferProbeResult, XrGpuU32ComputeProbeResult,
    ...
    os::linux::{
        ...
        vulkan_naga::{
            compile_compute_wgsl_to_spirv, CxVulkanShaderBinary, CxVulkanShaderDescriptorKind,
        },
    },
...
struct VulkanXrU32ComputeProbeResources {
    input: VulkanBuffer,
    output: VulkanBuffer,
...
struct VulkanXrF32ForceProbeResources {
    input: VulkanBuffer,
    output: VulkanBuffer,
    shader_module: vk::ShaderModule,
    descriptor_set_layout: vk::DescriptorSetLayout,
...
struct VulkanXrF32SkinningProbeResources {
    input: VulkanBuffer,
    output: VulkanBuffer,
    shader_module: vk::ShaderModule,
    descriptor_set_layout: vk::DescriptorSetLayout,
...
struct VulkanXrStorageBufferProbeResources {
...
impl CxVulkan {
    ...
    pub(crate) fn submit_xr_f32_skinning_probe(
        ...
        let read_result = if command_result.is_ok() {
            unsafe {
                match self.device.map_memory(
                    ...
                    Err(err) => Err(format!(
                        "map_memory(f32 skinning probe readback) failed: {err:?}"
                    )),
                }
            }
        } else {
            Err(command_result
                .err()
                .unwrap_or_else(|| "unknown f32 skinning probe command failure".to_string()))
        };
        ...
        let output_positions = read_result?;
        let mut mismatched_components = 0;
        let mut max_abs_error = 0.0_f32;
        for index in 0..sample_count {
            for component in 0..3 {
    ...
    fn destroy_xr_u32_compute_probe_resources(&mut self) {
    ...
    fn destroy_xr_f32_force_probe_resources(&mut self) {
    ...
    fn destroy_xr_f32_skinning_probe_resources(&mut self) {
    ...
    fn destroy_xr_storage_buffer_probe_resources(&mut self) {
