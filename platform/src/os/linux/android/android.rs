...
#[path = "android_java_messages.rs"]
mod android_java_messages;
...
impl CxOsApi for Cx {
    ...
    fn xr_gpu_f32_skinning_probe(
        ...
        {
            let vulkan = self.os.vulkan.as_mut()?;
            match vulkan.submit_xr_f32_skinning_probe(samples, sample_count, tolerance) {
                Ok(result) => Some(result),
                Err(err) => {
                    crate::warning!("OpenXR Vulkan f32 skinning probe failed: {err}");
                    None
                }
            }
        }
    ...
    fn xr_gpu_f32_mesh_sdf_probe_submit(
        ...
        {
            let vulkan = self.os.vulkan.as_mut()?;
            match vulkan.submit_xr_f32_mesh_sdf_probe_async(
                vertices,
                triangles,
                grid,
                sample_linear_indices,
                ...
                Ok(ticket) => Some(ticket),
                Err(err) => {
                    crate::warning!("OpenXR Vulkan f32 mesh SDF probe submit failed: {err}");
                    None
                }
            }
        }
    ...
    fn xr_gpu_f32_mesh_sdf_probe_poll(
        ...
        {
            let vulkan = self.os.vulkan.as_mut()?;
            match vulkan.poll_xr_f32_mesh_sdf_probe(request_id) {
                Ok(result) => result,
                Err(err) => {
    ...
    fn xr_gpu_f32_field_sample_probe(
        ...
        {
            let vulkan = self.os.vulkan.as_mut()?;
            match vulkan.submit_xr_f32_field_sample_probe(
                sample_linear_indices,
                expected_distances,
                sample_count,
                tolerance,
                ...
                Ok(result) => Some(result),
                Err(err) => {
                    crate::warning!("OpenXR Vulkan f32 field sample probe failed: {err}");
                    None
                }
            }
        }
...
pub struct CxOs {
    ...
    /// Last `TextInputConfig` sent to the Android IME. Cleared when the
    /// keyboard is requested hidden or Java reports it closed.
