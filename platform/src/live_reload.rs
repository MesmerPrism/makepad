...
fn collect_compiled_sites_for_file(
    ...
    for body in bodies.iter() {
        ...
        // `script_apply_eval!` bodies are tagged with `__script_source__`.
        // They are Rust-driven runtime evals, not static `script_mod!` blocks,
        // so the source extractor cannot match them during file hot reload.
        if script_mod
            .code
            .trim_start()
