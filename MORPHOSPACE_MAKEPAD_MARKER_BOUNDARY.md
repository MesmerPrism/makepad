...
| `rusty.quest.makepad.gpu_field_sampling_probe.v1` / `RUSTY_QUEST_MAKEPAD_GPU_FIELD_SAMPLING_PROBE` | `Cx::xr_gpu_f32_field_sample_probe` returning `XrGpuF32FieldSampleProbeResult` | Makepad may expose bounded sampling of the resident dense-SDF buffer produced by the mesh-SDF proof. Quest-Makepad owns Matter CPU-oracle distances and the marker contract; Makepad must stay a generic Vulkan adapter and must not claim field authority, force authority, or high-rate JSON payload transport. |
| `rusty.quest.makepad.gpu_field_force_sampling_probe.v1` / `RUSTY_QUEST_MAKEPAD_GPU_FIELD_FORCE_SAMPLING_PROBE` | `Cx::xr_gpu_f32_field_force_sample_probe` returning `XrGpuF32FieldForceSampleProbeResult` | Makepad may expose bounded acceleration sampling from the resident dense-SDF buffer using supplied sample positions and oracle accelerations. Quest-Makepad owns force semantics and CPU-oracle comparison; Makepad must keep the probe non-authoritative and must not become the runtime particle integrator or force-mode selector. |
| `rusty.quest.makepad.stimulus_volume_gpu_probe.v1` / `RUSTY_QUEST_MAKEPAD_STIMULUS_VOLUME_GPU_PROBE` | `Cx::xr_gpu_f32_volume_probe_submit` / `Cx::xr_gpu_f32_volume_probe_poll` returning `XrGpuF32VolumeProbeResult` | Makepad may expose a bounded XR/Vulkan f32 volume point-sample compute/readback proof. Quest-Makepad owns the Optics profile/oracle marker semantics; Makepad must not define stimulus profile authority, runtime readiness, or high-rate volume payload transport. |
| `rusty.quest.makepad.stimulus_volume_raymarch_preview.v1` / `RUSTY_QUEST_MAKEPAD_STIMULUS_VOLUME_RAYMARCH_PREVIEW` | `Cx::xr_gpu_f32_volume_raymarch_preview_submit` / `Cx::xr_gpu_f32_volume_raymarch_preview_poll` returning `XrGpuF32VolumeRaymarchPreviewResult` | Makepad may expose a tiny stereo storage-buffer raymarch output readback proof. Quest-Makepad owns the Optics profile/oracle marker and must keep `gpuComputeReady=false` until runtime-scale evidence exists; Makepad must not claim a bound runtime texture or production volume renderer. |
| `rusty.quest.makepad.stimulus_volume_image_preview.v1` / `RUSTY_QUEST_MAKEPAD_STIMULUS_VOLUME_IMAGE_PREVIEW` | `Cx::xr_gpu_f32_volume_image_preview_submit` / `Cx::xr_gpu_f32_volume_image_preview_poll` / `Cx::xr_gpu_f32_volume_image_preview_adopt_texture` returning `XrGpuF32VolumeImagePreviewResult` | Makepad may expose a scalable stereo image-atlas compute/readback and texture-adoption proof. Quest-Makepad owns the Optics profile/oracle marker, atlas semantics, and readiness fields; Makepad must not define stimulus profile authority, downstream tuning policy, or high-rate volume payload transport. |
