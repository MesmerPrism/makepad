...
impl LoopThreadConfig {
    ...
    fn new_impl(m: usize, k: usize, n: usize, max_threads: usize, kmc: usize) -> Self {
        ...
        let arch_factor = if cfg!(target_arch = "arm") { 20 } else { 1 };
        ...
        LoopThreadConfig { loop3, loop2 }
    }

    /// Number of packing buffers for A
    #[inline(always)]
    pub(crate) fn num_pack_a(&self) -> usize {
        self.loop3 as usize
    }
}
    ...
    G: Fn(usize, usize) -> R + Sync,
{
        ...
        F: Fn(ThreadPoolCtx<'_>, &mut R, usize, usize) + Sync,
    {
        ...
        debug_assert!(
            self.nthreads <= 4,
            "this method does not support nthreads > 4, got {}",
            self.nthreads
        );
        let pool = self.pool;
