...
use crate::threading::{get_thread_pool, LoopThreadConfig, ThreadPoolCtx};
#[cfg(feature = "cgemm")]
...
pub unsafe fn sgemm(
    ...
    sgemm_kernel::detect(GemmParameters {
        m,
        k,
        n,
        alpha,
...
pub unsafe fn dgemm(
    ...
    dgemm_kernel::detect(GemmParameters {
        m,
        k,
        n,
        alpha,
...
pub unsafe fn cgemm(
    ...
    let _ = (flaga, flagb);
    cgemm_kernel::detect(GemmParameters {
        m,
        k,
        n,
        alpha,
...
pub unsafe fn zgemm(
    ...
    let _ = (flaga, flagb);
    zgemm_kernel::detect(GemmParameters {
        m,
        k,
        n,
        alpha,
...
impl<T> GemmSelect<T> for GemmParameters<T> {
        ...
        T: Element,
    {
        ...
        unsafe { gemm_loop::<K>(m, k, n, alpha, a, rsa, csa, b, rsb, csb, beta, c, rsc, csc) }
    }
}
    ...
    for (l5, nc) in range_chunk(n, knc) {
        ...
        for (l4, kc) in range_chunk(k, kkc) {
            ...
            K::pack_nr(
                kc,
                nc,
                slice::from_raw_parts_mut(bpp.ptr(), bp_size),
                b.ptr(),
                csb,
                rsb,
            );
            ...
            range_chunk(m, kmc)
                ...
                .for_each(move |tp, &mut app, l3, mc| {
                    ...
                    K::pack_mr(
                        kc,
                        mc,
                        slice::from_raw_parts_mut(app.ptr(), ap_size),
                        a.ptr(),
                        rsa,
                        csa,
                    );
...
/// + mc: rows of packed A
unsafe fn gemm_packed<K>(
    nc: usize,
    kc: usize,
    mc: usize,
    alpha: K::Elem,
    app: Ptr<*const K::Elem>,
    bpp: Ptr<*const K::Elem>,
    beta: K::Elem,
    c: Ptr<*mut K::Elem>,
    rsc: isize,
    csc: isize,
    tp: ThreadPoolCtx,
    thread_config: LoopThreadConfig,
) where
    K: GemmKernel,
{
    let mr = K::MR;
    ...
    #[cfg(not(feature = "std"))]
    let mut mask_buf = MaskBuffer {
        buffer: [0; MASK_BUF_SIZE],
    };
...
unsafe fn make_packing_buffer<K>(
    m: usize,
    k: usize,
    n: usize,
    na: usize,
...
#[inline]
unsafe fn c_to_masked_ab_beta_c<T, K>(
    beta: T,
    c: *mut T,
    rsc: isize,
    csc: isize,
    ...
    T: Element,
{
    ...
    for j in 0..nr {
        for i in 0..mr {
            if i < rows && j < cols {
                let cptr = c.stride_offset(rsc, i).stride_offset(csc, j);
                if beta.is_zero() {
                    *cptr = *ab; // initialize
                } else {
...
// Compute just C ← βC
#[inline(never)]
unsafe fn c_to_beta_c<T>(m: usize, n: usize, beta: T, c: *mut T, rsc: isize, csc: isize)
where
    T: Element,
{
    for i in 0..m {
